...
} from './Code_DQ70Dgra.mjs';
import {
  I as cssesc_1,
  J as getDefaultExportFromCjs,
} from './astro/server_BB8JYHJv.mjs';
...
var processorExports = processor$1.exports;
...
const { AtRule, Rule } = postcss_1;
let parser = distExports;
...
const postcssNested =
  /*@__PURE__*/ getDefaultExportFromCjs(postcssNestedExports);
...
function getInlineStyles(node) {
  ...
  try {
    ...
    root.each((node2) => {
      if (node2.type === 'decl') styles.set(node2.prop, node2.value);
    });
  } catch (error) {}
  return styles;
}
...
var AnnotationRenderPhaseOrder = [
  'earliest',
  'earlier',
  'normal',
  'later',
...
var InlineStyleAnnotation = class extends ExpressiveCodeAnnotation {
  ...
  render({ nodesToTransform, styleVariants }) {
    ...
    const variantIndices =
      this.styleVariantIndex !== void 0
        ? [this.styleVariantIndex]
        : styleVariants.map((_, i) => i);
    ...
    const isInlineStyleNode = (node) =>
      node.tagName === 'span' && // Our inline style nodes have no class names
    ...
    const modifyExistingStyles = (node, remove = false) => {
      const existingStyles = (node.properties?.style?.toString() || '')
        .split(';')
        .map((style) => {
...
var MetaOptions = class {
  ...
  list(keyOrKeys, kind) {
    const filtered = this.#parsedOptions.filter((option) => {
      if (kind !== void 0 && option.kind !== kind) return false;
      if (keyOrKeys === void 0) return true;
...
function parseDelimitedValues(input, syntax) {
  ...
  const regExpParts = valueDelimiterPairs.map(
    ({ valueStartDelimiter, valueEndDelimiter }) => {
      const part = [
  ...
  return matches2.map((match) => {
    ...
    const firstCaptureGroupIndex = keyValuePairs.findIndex(
      (value2) => value2 !== void 0,
    );
    const delimiterPairIdx = Math.floor(firstCaptureGroupIndex / 2);
    const { valueStartDelimiter, valueEndDelimiter } =
      valueDelimiterPairs[delimiterPairIdx];
    const [key, escapedValue] = keyValuePairs.slice(
      delimiterPairIdx * 2,
      delimiterPairIdx * 2 + 2,
    );
...
function renderLineToAst({ line, lineIndex, gutterElements, ...restContext }) {
  const { textParts, partIndicesByAnnotation } =
    splitLineAtAnnotationBoundaries(line);
  ...
  annotations.forEach((annotation, annotationIndex) => {
    ...
    if (!partIndices)
      throw new Error(
        `Failed to find inline annotation in part indices: ${JSON.stringify(annotation)}`,
      );
    if (partIndices.length > 1) {
      const isPartiallyContainedInLaterAnnotations = annotations
        .slice(annotationIndex + 1)
        .some((laterAnnotation) => {
          ...
          const intersectingParts = laterPartIndices.filter((partIndex) =>
            partIndices.includes(partIndex),
          );
          const isPartiallyContained =
            intersectingParts.length > 0 &&
            intersectingParts.length < partIndices.length;
      ...
      if (!isPartiallyContainedInLaterAnnotations) {
        ...
        partIndicesByAnnotation.forEach((partIndicesToCheck) => {
          ...
          if (anyChanges) {
            partIndicesToCheck.splice(
              0,
              partIndicesToCheck.length,
              ...updatedIndices,
            );
          }
        });
    ...
    const renderOutput = annotation.render({
      nodesToTransform: [...renderInput],
      line,
      lineIndex,
      ...restContext,
    });
  ...
  const sortedGutterElements = [...gutterElements].sort((a, b) =>
    renderPhaseSortFn(a.gutterElement, b.gutterElement),
  );
  const renderedGutterElements = sortedGutterElements.map(
    ({ pluginName, gutterElement }) => {
      try {
        const node = gutterElement.renderLine({
          ...restContext,
          line,
          lineIndex,
        });
        if (!isHastElement(node))
          throw new Error(
            `renderLine function did not return a valid HAST Element node: ${JSON.stringify(node)}`,
          );
        return node;
      } catch (error) {
        const msg = error instanceof Error ? error.message : error;
        throw new Error(
          `Plugin "${pluginName}" failed to render a gutter element. Error message: ${msg}`,
          { cause: error },
        );
      }
    },
    ...
    const renderedGutterElements = sortedGutterElements.map(
      ({ pluginName, gutterElement }) => {
        try {
          ...
          if (!isHastElement(node))
            throw new Error(
              `renderPlaceholder function did not return a valid HAST Element node: ${JSON.stringify(node)}`,
            );
          return node;
        } catch (error) {
          const msg = error instanceof Error ? error.message : error;
          throw new Error(
            `Plugin "${pluginName}" failed to render a gutter element placeholder. Error message: ${msg}`,
            { cause: error },
          );
        }
      },
...
function validateAnnotationRenderOutput(nodes, expectedLength) {
  if (!Array.isArray(nodes) || nodes.length !== expectedLength)
    throw new Error(
      `Expected annotation render function to return an array of ${expectedLength} node(s), but got ${JSON.stringify(nodes)}.`,
    );
  nodes.forEach((node, nodeIndex) => {
    if (!node || !node.type)
      throw new Error(
        `Annotation render function returned an invalid node at index ${nodeIndex}: ${JSON.stringify(node)}`,
      );
  });
}
    ...
    await runHooks(
      ...
      async ({ hookFn, plugin }) => {
        ...
        if (!isHastElement(lineRenderData.lineAst)) {
          throw newTypeError('hast Button', lineRenderData.lineAst, 'lineAst');
        }
      },
    );
  ...
  await runHooks(
    ...
    async ({ hookFn, plugin }) => {
      ...
      if (!isHastElement(blockRenderData.blockAst)) {
        throw newTypeError('hast Button', blockRenderData.blockAst, 'blockAst');
      }
    },
  );
  return {
...
function validateExpressiveCodeProcessingState(state) {
  const isValid =
    state && // Expect all properties to be defined and booleans
...
function getAbsoluteRange({ start, end, rangeMax }) {
...
var ExpressiveCodeLine = class {
  ...
  set parent(value) {
    ...
    if (this.#parent) {
      ...
      throw new Error(
        `You cannot change the parent of a code line after it has been added to a code block.`,
      );
    }
    this.#parent = value;
  }
  #annotations = [];
  getAnnotations() {
    const matchingAnnotations = this.#annotations.filter(
  ...
  editText(columnStart, columnEnd, newText) {
    ...
    for (let index = this.#annotations.length - 1; index >= 0; index--) {
      ...
      const { columnStart: annotationStart, columnEnd: annotationEnd } =
    ...
    this.#text =
      this.text.slice(0, editStart) + newText + this.text.slice(editEnd);
    return this.text;
  }
};
...
var ExpressiveCodeBlock = class {
  ...
  set language(value) {
    if (this.#state?.canEditLanguage === false)
      throw new Error(
        'Cannot edit code block property "language" in the current state.',
      );
    this.#language = value;
  }
  get meta() {
  ...
  set meta(value) {
    if (this.#state?.canEditMetadata === false)
      throw new Error(
        'Cannot edit code block property "meta" in the current state.',
      );
    this.#meta = value;
  ...
  set state(value) {
    ...
    if (this.#state) {
      ...
      throw new Error(
        `You cannot change the state object of a code block after assigning it once.`,
      );
    }
    this.#state = value;
  }
  ...
  deleteLines(indices) {
    ...
    sorted.forEach((index) => {
      if (lastIndex === index)
        throw new Error(
          `A batch of lines to delete cannot contain the same index twice. Given indices: ${JSON.stringify(indices)}`,
        );
      lastIndex = index;
      const isValidIndex = index >= 0 && index < this.#lines.length;
      if (!isValidIndex)
        throw new Error(
          `Cannot delete invalid index ${JSON.stringify(index)} from line array (length=${this.#lines.length}). Given indices: ${JSON.stringify(indices)}`,
        );
  ...
  insertLines(index, textLines) {
    ...
    if (!isValidIndex)
      throw new Error(
        `Cannot insert at invalid index ${JSON.stringify(index)} into line array (length=${this.#lines.length}).`,
      );
...
var processor = postcss([
  ...
  (root) => {
    ...
    const regExpScopedTopLevel = new RegExp(
      `^${groupWrapperScope} .*(${groupWrapperScope}|:root|html|body)`,
      'g',
    );
    root.walkRules((rule) => {
      rule.selectors = rule.selectors.map((selector) =>
        selector.replace(regExpScopedTopLevel, '$1'),
      );
    });
  },
...
async function renderGroup({
  ...
  const groupRenderData = {
    groupAst: buildGroupAstFromRenderedBlocks(
  ...
  await runHooks(
    ...
    async ({ hookFn, plugin }) => {
      await hookFn({
        ...
        pluginStyles,
        addStyles: (styles) =>
          pluginStyles.push({ pluginName: plugin.name, styles }),
        renderData: groupRenderData,
      });
      if (!isHastElement(groupRenderData.groupAst)) {
        throw newTypeError('hast Button', groupRenderData.groupAst, 'groupAst');
      }
    },
  );
  return {
...
var convertLabToLch = ({ l, a, b, alpha }, mode = 'lch') => {
  ...
  let res = { mode, l, c };
  if (c) res.h = normalizeHue_default((Math.atan2(b, a) * 180) / Math.PI);
  if (alpha !== void 0) res.alpha = alpha;
  return res;
};
...
var convertOklabToRgb = (c) =>
  convertLrgbToRgb_default(convertOklabToLrgb_default(c));
...
function binarySearch({
  ...
  while (((mid = (low + high) / 2), iterations < maxIterations)) {
    ...
    const resultIsWithinTolerance =
      Math.abs(currentValue - targetValue) <= tolerance;
    const resultIsInPreferredDirection =
      preferHigher === void 0
        ? true
        : preferHigher
          ? currentValue > targetValue
          : currentValue < targetValue;
    const midChangedLessThanEpsilon =
      lastMid !== void 0 && Math.abs(lastMid - mid) < epsilon;
    if (
      resultIsInPreferredDirection &&
      (resultIsWithinTolerance || midChangedLessThanEpsilon)
    ) {
      return mid;
...
function xyzToRgb(xyz) {
  ...
  const linearToSrgb = (v) =>
    v > 31308e-7 ? 1.055 * Math.pow(v, 1 / 2.4) - 0.055 : 12.92 * v;
...
function setLuminance(input, targetLuminance) {
  return withParsedColor(input, (color) => {
    ...
    const mixAmount = binarySearch({
      ...
      preferHigher:
        targetLuminance > 0 && targetLuminance < 1 ? increasing : void 0,
      tolerance: 1 / 256,
...
function ensureColorContrastOnBackground(
  input,
  background,
  minContrast = 5.5,
  maxContrast = 22,
  ...
  return withParsedColor(input, (color) => {
    return withParsedColor(
      ...
      (backgroundColor) => {
        ...
        let curContrast = readability(
          newColor.onBackground(backgroundColor),
          backgroundColor,
        );
...
function changeAlphaToReachColorContrast(
  input,
  background,
  minContrast = 6,
  maxContrast = 22,
...
function chromaticRecolor(input, target) {
  ...
  return withParsedColor(input, (color) => {
    ...
    oklch.h = targetHue;
    const maxChromaForInputLightness = rgbaToOklch(
      oklchToRgba({ ...oklch, c: 0.4 }),
    ).c;
    let newChroma;
    if (targetChromaMeasuredAtLightness !== void 0) {
      const maxChromaForTargetLightness = rgbaToOklch(
        oklchToRgba({ ...oklch, c: 0.4, l: targetChromaMeasuredAtLightness }),
      ).c;
      const relativeTargetChroma =
        Math.min(targetChroma, maxChromaForTargetLightness) /
        maxChromaForTargetLightness;
      newChroma = maxChromaForInputLightness * relativeTargetChroma;
    } else {
      newChroma = Math.min(targetChroma, maxChromaForInputLightness);
    }
    const linearDecrease = (i, start, end) =>
      Math.max(0, Math.min(1, 1 - (i - start) / (end - start)));
...
function resolveVSCodeWorkbenchColors(colors, themeType) {
  ...
  function applyColorTransform(unresolvedColor) {
    ...
    if (
      unresolvedColor.length === 5 &&
      unresolvedColor[0] === 'lessProminent'
    ) {
  ...
  function resolveColor(unresolvedColor) {
    ...
    if (alreadyResolvedColor !== void 0) return alreadyResolvedColor;
    ...
    if (resolved === void 0)
      throw new Error(
        `Invalid color value ${JSON.stringify(unresolvedColor)}, expected a hex color.`,
      );
...
function guessThemeTypeFromEditorColors(colors) {
  const bgLuminance = getLuminance(
    colors?.['editor.background'] || defaultEditorBackgroundColors[0],
  );
  const fgLuminance = getLuminance(
    colors?.['editor.foreground'] || defaultEditorForegroundColors[0],
  );
...
var ExpressiveCodeTheme = class _ExpressiveCodeTheme {
  ...
  applyHueAndChromaAdjustments(adjustments) {
    ...
    if (adjustments.backgrounds) {
      adjustColors(
        groupedDefaultWorkbenchColorKeys.backgrounds,
        adjustments.backgrounds,
      );
    }
    if (adjustments.accents) {
      adjustColors(
        groupedDefaultWorkbenchColorKeys.accents,
        adjustments.accents,
      );
    }
  ...
  ensureMinSyntaxHighlightingColorContrast(minContrast = 5.5, backgroundColor) {
    ...
    const fixContrast = (color) => {
      ...
      const newColor = ensureColorContrastOnBackground(
        color,
    ...
    this.settings.forEach((s2) => {
      if (!s2.settings.foreground) return;
  ...
  parseThemeSettings(settings) {
    ...
    return settings.map((unknownSetting) => {
      ...
      const scope = Array.isArray(anyScope)
        ? anyScope.slice()
  ...
  static fromJSONString(json) {
    return new _ExpressiveCodeTheme(
      JSON.parse(stripJsonComments(json, { trailingCommas: true })),
    );
  }
};
...
function getCoreBaseStyles({
  ...
  const ifThemedSelectionColors = (css) =>
...
function resolveStyleSettings({
  ...
  const unresolvedByPath = getStyleSettingsByPath(
    coreStyleSettings.defaultValues,
  );
  plugins.forEach((plugin) => {
    if (!plugin.styleSettings) return;
    applyStyleSettings(
      unresolvedByPath,
      getStyleSettingsByPath(plugin.styleSettings.defaultValues),
    );
  });
  applyStyleSettings(
    unresolvedByPath,
    getStyleSettingsByPath(styleOverrides ?? {}),
  );
  applyStyleSettings(
    unresolvedByPath,
    getStyleSettingsByPath(theme.styleOverrides ?? {}),
  );
  function resolveSetting(settingPath) {
    ...
    if (result === void 0 && !resolvedByPath.has(settingPath)) {
      if (attemptedToResolve.has(settingPath))
        throw new Error(
          `Circular dependency detected while resolving style setting '${settingPath}'`,
        );
...
function getCssVarDeclarations({ resolvedStyleSettings, plugins, cssVarName }) {
  const cssVarDeclarations = /* @__PURE__ */ new Map();
  const excludedPaths = /* @__PURE__ */ new Set();
  plugins.forEach((plugin) => {
    plugin.styleSettings?.cssVarExclusions.forEach((path) =>
...
function resolveStyleVariants({ themes, plugins, styleOverrides, cssVarName }) {
  return themes.map((theme, styleVariantIndex) => {
    const resolvedStyleSettings = resolveStyleSettings({
      theme,
      styleVariantIndex,
      plugins,
      styleOverrides,
    });
    const cssVarDeclarations = getCssVarDeclarations({
...
var tabindex_js_module_min_default =
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          if (indent > 0) {
            line.getAnnotations().forEach((annotation) => {
              ...
              if (
                inlineRange.columnStart >= 0 &&
                inlineRange?.columnEnd <= indent
              ) {
...
var ExpressiveCodeEngine = class {
  ...
  constructor(config) {
    ...
    if (deprecatedConfig.theme && !config.themes) {
      config.themes = Array.isArray(deprecatedConfig.theme)
        ? deprecatedConfig.theme
        : [deprecatedConfig.theme];
      delete deprecatedConfig.theme;
    }
    this.themes = Array.isArray(config.themes)
      ? [...config.themes]
      : config.themes
        ? [config.themes]
        : [
    ...
    this.minSyntaxHighlightingColorContrast =
      config.minSyntaxHighlightingColorContrast ?? 5.5;
  ...
  async getThemeStyles() {
    ...
    const { cssVarDeclarations: baseVars, theme: baseTheme } =
      this.styleVariants[0];
    const baseThemeSelector =
      this.themeCssSelector &&
    ...
    for (
      let styleVariantIndex = 1;
      styleVariantIndex < this.styleVariants.length;
      styleVariantIndex++
    ) {
    ...
    if (this.themeCssSelector !== false) {
      for (const { theme, cssVars, coreStyles } of alternateVariants) {
        const themeSelector =
          this.themeCssSelector &&
          this.themeCssSelector(theme, { styleVariants: this.styleVariants });
  ...
  getResolverContext() {
    return {
      cssVar: (styleSetting, fallbackValue) =>
        this.cssVar(styleSetting, fallbackValue),
      cssVarName: getCssVarName,
...
var PluginTexts = class {
  ...
  overrideTexts(locale, localeTextOverrides) {
    ...
    const localeOverrides =
      this.overridesByLocale.get(locale) ||
      this.overridesByLocale.set(locale, {}).get(locale);
  ...
  applyOverrides(texts, acceptedLocales) {
    const result = { ...texts };
    const overrides = [...acceptedLocales, void 0]
      .map((locale) => this.overridesByLocale.get(locale))
      .filter((x) => x);
    if (overrides.length) {
...
var framesStyleSettings = new PluginStyleSettings({
  defaultValues: {
    frames: {
      shadowColor: ({ theme, resolveSetting }) =>
        theme.colors['widget.shadow'] ||
        multiplyAlpha(resolveSetting('borderColor'), 0.75),
      frameBoxShadowCssValue: ({ resolveSetting }) =>
        `0.1rem 0.1rem 0.2rem ${resolveSetting('frames.shadowColor')}`,
      ...
      editorActiveTabIndicatorHeight: ({ resolveSetting }) =>
      ...
      editorTabBorderRadius: ({ resolveSetting }) =>
        resolveSetting('borderRadius'),
      editorTabBarBackground: ({ theme }) =>
        theme.colors['editorGroupHeader.tabsBackground'],
      editorTabBarBorderColor: ({ resolveSetting }) =>
        resolveSetting('borderColor'),
      editorTabBarBorderBottomColor: ({ theme }) =>
        theme.colors['editorGroupHeader.tabsBorder'] || 'transparent',
      editorBackground: ({ resolveSetting }) =>
        resolveSetting('codeBackground'),
      terminalTitlebarDotsForeground: ({ resolveSetting }) =>
        resolveSetting('frames.terminalTitlebarForeground'),
      ...
      terminalTitlebarBorderBottomColor: ({ theme, resolveSetting }) =>
        ...
        onBackground(
          resolveSetting('borderColor'),
          theme.type === 'dark' ? '#000000bf' : '#ffffffbf',
        ),
      terminalBackground: ({ theme }) => theme.colors['terminal.background'],
      inlineButtonBackground: ({ resolveSetting }) =>
        resolveSetting('frames.inlineButtonForeground'),
      ...
      inlineButtonForeground: ({ resolveSetting }) =>
        resolveSetting('codeForeground'),
      inlineButtonBorder: ({ resolveSetting }) =>
        resolveSetting('frames.inlineButtonForeground'),
      inlineButtonBorderOpacity: '0.4',
      tooltipSuccessBackground: ({ theme }) =>
        setLuminance(theme.colors['terminal.ansiGreen'] || '#0dbc79', 0.18),
...
var LanguagesWithFencedFrontmatter = [
  'astro',
  'markdown',
  'md',
  'mdx',
...
function getFileNameFromComment(line, lang) {
  ...
  const hasTypicalFileNameBeginning = possibleFileName.match(
    /^(\/|\\|\.[/\\]|~|[a-z]:).+/i,
  );
  ...
  const hasTypicalFileNamePattern =
    hasTypicalFileNameBeginning ||
    hasFileNameStartingWithDot ||
    looksLikeSeparatedPath;
  if (
    hasTypicalFileNamePattern &&
    (!textBeforeFileName.length || languageGroup === LanguageGroups.terminal)
  ) {
...
function extractFileNameFromCodeBlock(codeBlock) {
  ...
  if (LanguagesWithFencedFrontmatter.includes(codeBlock.language)) {
    const openingFence =
      lineIdx > 0 ? codeBlock.getLine(lineIdx - 1)?.text.trim() : void 0;
    const closingFence = codeBlock.getLine(lineIdx)?.text.trim();
    const isFrontmatterEmptyNow =
      openingFence === closingFence &&
      ['---', '+++'].includes(openingFence ?? '');
...
var copy_js_module_min_default =
...
function pluginFrames(options = {}) {
  ...
  return {
    ...
    jsModules: options.showCopyToClipboardButton
      ? [
          copy_js_module_min_default.replace(
            /\[SELECTOR\]/g,
    ...
    hooks: {
      ...
      preprocessCode: ({ codeBlock }) => {
        ...
        if (
          props.title === void 0 &&
          props.frame !== 'none' &&
          options.extractFileNameFromCode
        ) {
          props.title = extractFileNameFromCodeBlock(codeBlock);
        }
        if (
          (props.frame ?? 'auto') === 'auto' &&
          isTerminalLanguage(language)
        ) {
          const titleIsFileName =
            props.title &&
            getFileNameFromComment(`// ${props.title}`, language);
          if (
            titleIsFileName ||
            codeBlock.getLines(0, 4).some((line) => line.text.match(/^\s*#!/))
          ) {
            props.frame = 'code';
          }
        }
      ...
      postprocessRenderedBlock: ({ codeBlock, renderData, locale }) => {
        ...
        const visibleTitle =
          (frame !== 'none' && titleText) || isTerminal
            ? [h('span', { className: 'title' }, titleText || '')]
            : [];
        const screenReaderTitle =
          !titleText && isTerminal
            ? [
                h(
                  'span',
                  { className: 'sr-only' },
                  texts.terminalWindowFallbackTitle,
                ),
              ]
            : [];
        const extraElements = [];
  ...
  defaultValues: {
    textMarkers: {
      ...
      markBorderColor: (context) =>
        resolveBorder(context, 'textMarkers.markHue'),
      ...
      insDiffIndicatorColor: (context) =>
        resolveIndicator(context, 'textMarkers.insHue'),
      ...
      delDiffIndicatorColor: (context) =>
        resolveIndicator(context, 'textMarkers.delHue'),
    },
  },
  cssVarExclusions: [
...
function resolveBg({ resolveSetting: r }, hue) {
  return toHexColor(
    `lch(${r('textMarkers.defaultLuminance')} ${r('textMarkers.defaultChroma')} ${r(hue)} / ${r('textMarkers.backgroundOpacity')})`,
  );
}
function resolveBorder({ resolveSetting: r }, hue) {
  return toHexColor(
    `lch(${r('textMarkers.borderLuminance')} ${r('textMarkers.defaultChroma')} ${r(hue)} / ${r('textMarkers.borderOpacity')})`,
  );
}
function resolveIndicator({ resolveSetting: r }, hue) {
  return toHexColor(
    `lch(${r('textMarkers.indicatorLuminance')} ${r('textMarkers.defaultChroma')} ${r(hue)} / ${r('textMarkers.indicatorOpacity')})`,
  );
}
    ...
    toDefinitionsArray(codeBlock.props[markerType]).forEach((definition) => {
      ...
      if (definition instanceof RegExp) {
        ...
        for (const match of matches) {
          ...
          let groupIndices = rawGroupIndices.flatMap((range) =>
            range ? [range] : [],
          );
  ...
  return {
    ...
    hooks: {
      ...
      preprocessMetadata: ({ codeBlock, cssVar }) => {
        ...
        codeBlock.props.useDiffSyntax =
          codeBlock.metaOptions.getBoolean('useDiffSyntax') ??
          codeBlock.props.useDiffSyntax;
        MarkerTypeOrder.forEach((markerType) => {
          toDefinitionsArray(codeBlock.props[markerType]).forEach(
            (definition) => {
              if (
              ...
              const lineNumbers = (0, import_parse_numeric_range.default)(
                range,
              );
              lineNumbers.forEach((lineNumber, idx) => {
                ...
                codeBlock.getLine(lineIndex)?.addAnnotation(
                  new TextMarkerAnnotation({
                    ...
                    label:
                      idx === 0 || lineNumber - lineNumbers[idx - 1] !== 1
                        ? label
                        : void 0,
                  }),
                );
      ...
      preprocessCode: ({ codeBlock, cssVar }) => {
        if (codeBlock.language === 'diff' || codeBlock.props.useDiffSyntax) {
          const lines = codeBlock.getLines();
          const couldBeRealDiffFile = lines
            .slice(0, 4)
            .some((line) =>
              line.text.match(/^([*+-]{3}\s|@@\s|[0-9,]+[acd][0-9,]+\s*$)/),
            );
          if (!couldBeRealDiffFile) {
            let minIndentation = Infinity;
            const parsedLines = lines.map((line) => {
              const [, indentation, marker, content] =
                line.text.match(/^(([+-](?![+-]))?\s*)(.*)$/) || [];
              const markerType =
                marker === '+' ? 'ins' : marker === '-' ? 'del' : void 0;
              if (
                content.trim().length > 0 &&
                indentation.length < minIndentation
              )
      ...
      postprocessAnnotations: ({ codeBlock, styleVariants, config }) => {
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          for (const annotation of annotations) {
            ...
            if (fullLineMarker) {
              if (
                MarkerTypeOrder.indexOf(annotation.markerType) <
                MarkerTypeOrder.indexOf(fullLineMarker.markerType)
              )
                continue;
              if (
                AnnotationRenderPhaseOrder.indexOf(
                  annotation.renderPhase || 'normal',
                ) <
                AnnotationRenderPhaseOrder.indexOf(
                  fullLineMarker.renderPhase || 'normal',
                )
              )
                continue;
            }
          ...
          styleVariants.forEach((styleVariant, styleVariantIndex) => {
            const fullLineMarkerBgColor =
              (fullLineMarker &&
                styleVariant.resolvedStyleSettings.get(
                  markerBgColorPaths[fullLineMarker.markerType],
                )) ||
              'transparent';
            const lineBgColor = onBackground(
              fullLineMarkerBgColor,
              getStaticBackgroundColor(styleVariant),
            );
            const textColors = annotations.filter(
              (annotation) =>
                isInlineStyleAnnotation(annotation) &&
                annotation.color && // Only consider annotations that apply to the current style variant
                (annotation.styleVariantIndex === void 0 ||
            ...
            textColors.forEach((textColor) => {
              ...
              markers.forEach((marker) => {
                ...
                const markerEnd =
                  marker.inlineRange?.columnEnd ?? line.text.length;
                if (markerStart > textEnd || markerEnd < textStart) return;
                const inlineMarkerBgColor =
                  (marker.inlineRange &&
                    styleVariant.resolvedStyleSettings.get(
                      markerBgColorPaths[marker.markerType],
                    )) ||
                  'transparent';
                const combinedBgColor = onBackground(
                  inlineMarkerBgColor,
                  lineBgColor,
                );
                const readableTextColor = ensureColorContrastOnBackground(
                  textFgColor,
                  combinedBgColor,
                  config.minSyntaxHighlightingColorContrast,
                );
                if (
                  readableTextColor.toLowerCase() === textFgColor.toLowerCase()
                )
                  return;
                line.addAnnotation(
...
export {
  AnnotationRenderPhaseOrder as A,
  setAlpha as B,
  setLuminance as C,
  stableStringify as D,
  ExpressiveCodeTheme as E,
  toHexColor as F,
  validateExpressiveCodeAnnotation as G,
  LanguagesWithFencedFrontmatter as H,
  InlineStyleAnnotation as I,
  LanguageGroups as L,
  MetaOptions as M,
  PluginStyleSettings as P,
  ExpressiveCodeEngine as a,
  pluginFrames as b,
  pluginTextMarkers as c,
  ExpressiveCodeAnnotation as d,
  ExpressiveCodeBlock as e,
  ExpressiveCodeLine as f,
  getStableObjectHash as g,
  PluginTexts as h,
  changeAlphaToReachColorContrast as i,
  changeLuminanceToReachColorContrast as j,
  chromaticRecolor as k,
  codeLineClass as l,
  cssVarReplacements as m,
  darken as n,
  ensureColorContrastOnBackground as o,
  pluginFramesTexts as p,
  getCssVarName as q,
  getFirstStaticColor as r,
  getLuminance as s,
  getStaticBackgroundColor as t,
  isInlineStyleAnnotation as u,
  lighten as v,
  multiplyAlpha as w,
  onBackground as x,
  resolveStyleVariants as y,
  runHooks as z,
};
